...
打字時清單自己收斂，收斂到空的那一刻正好就是該問 AI 的時候。兩顆同級的章另外還
踩到「一個畫面只有一顆章」：那條的例外是**成對動作**（匯出／匯入，同一件事的兩個
方向），而「即時免費」和「要等要花錢」不是同一件事的兩個方向。
...
`SearchMode`（`SettingsStore.kt`）三選一，設定頁擺在「文字辨識用哪一家」正下方 ——
它們是同一個決定的兩半，分開放的話使用者得先猜「這個開關屬於誰」，而它其實兩家都影響。
