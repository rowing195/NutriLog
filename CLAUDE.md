...
同一個長相的收合狀態），點下去把五個入口**由上往下逐列落下**（`ROW_DROP_DP`，
排在上面的先落）。方向和設定選單那五列（由右進）刻意不同，理由見〈設定〉那節。**那顆按鈕整個是畫出來的、
...
`translationX` 從 28dp 收到 0、alpha 從 0 到 1，每列錯開一點。

**和今日頁那顆「記一筆」展開的五列刻意不同方向**（那邊是由上往下逐列落下）：
設定是報頭**右邊**那顆圖示開出來的，那五個是**右下角**那顆章開出來的 ——
方向不同才分得出「剛剛按的是哪一顆」。手法一樣（位移＋淡入＋錯開），
軸不一樣。
