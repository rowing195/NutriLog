...
**這是整段最容易被改壞的一行。** `matchScore`（`NutriViewModel.kt`）看的是關鍵字
**相鄰兩字**（bigram）的重疊比例，不是整串 `contains`。原因是中文沒有空白可以拆詞，
而使用者為了讓 AI 估得準，打的往往比庫裡存的更細：

| 打的 | 庫裡的 | 整串 `contains` | 相鄰兩字重疊 |
|---|---|---|---|
| 手沖藝妓黑咖啡 | 手沖黑咖啡 | ❌ 找不到 | 0.5 ✅ |
| 美式黑咖啡 | 手沖黑咖啡 | ❌ 找不到 | 0.5 ✅ |
| 黑咖啡 | 咖哩飯 | ❌ | 0.0 ✅ |

用相鄰兩字而不是單字，是因為單字會把「咖哩」也拉進來（共用一個「咖」）。
換回 `contains` 的症狀是**庫裡明明有相近品項，畫面卻理直氣壯地叫使用者去問 AI**
—— 而那正是這個畫面最該避免的事。**用英文測不出來**：英文有空白可以拆詞，
壞掉的是中文。

兩個數字不要亂動（都在 `matchScore` 附近，註解裡有理由）：

- `NEAR_MATCH_THRESHOLD = 0.3` 是唯一的旋鈕。調高會退回「打太細就找不到」，
  調低會開始推薦只共用一個常見詞的東西（「咖啡」誰都有）。
- `EXACT_MATCH_SCORE = 2.0` **必須大於 1.0**。近似分數的上限就是 1.0（所有相鄰
  兩字都湊得到、整串卻不在裡面），設成 1.0 的話兩者會撞在一起，「真的有這個」
  就不保證排在「長得有點像」前面。

**它沒有語意。** 「拿鐵」和「牛奶咖啡」一個字都不共用，就是配不起來；跨語言
（latte／拿鐵）也一樣。那種事只有 AI 做得到，不要試著在這裡補規則表。
