...
- **「回到本月」不在報頭裡，在畫面最底下，而且是空心章。** 它是一個動作而不是這個月
  ...
  進出要有動畫（從底邊往上展開）：它是跟著換月出現的，直接彈出來會像畫面閃了一下。
  **而它看的是 `currentPage`、不是 settle 後的 `month`** —— 前者一過半頁就變，
  手指還在拖它就開始動了；等 `month` 要等分頁器 fling 加吸附跑完，那是好幾百毫秒
  的延遲，看起來像慢半拍才想起來要出現。**這是「`currentPage` 只拿來畫、不拿去
  commit」那條規則的正確用法**：真正決定月份的仍然是 `settledPage` 那條 collector。
  進出都用兩頭收慢的標準曲線、都不留延遲：它是**就地長高縮矮**、不是從畫面外
  飛進來，用「進場減速、退場加速」那套會在起訖兩端留下看得到的稜角。

- **進場是逐週由上往下落下，而星期列不跟著動。** 星期列每個月都一樣，
  而且換月時它本來就釘在分頁器外面不滑 —— 進場讓它動等於同一個元件講兩套規則。
  概況接在最後一週後面當收尾，不接的話它會在上面幾列還在落的時候就先站好。
  **放慢的時候距離要跟著加**（現在是 340ms／18dp）：只延長時間不加距離，
  同一段路走更久，讀起來是「卡卡的」而不是「慢慢落下」。曲線用減速的
  `LinearOutSlowInEasing`：這是「落下」，落體不會先慢慢加速，而且起步慢的曲線
  配上放慢之後的時長，第一時間會讀成沒反應。
- **那個 `enter` transition 要存在畫面這一層，不能放進分頁頁面裡。** 它只翻一次，
  之後滑到別的月份是新組出來的頁，那時 transition 早就落定在 true，新的那幾列
  直接是最終狀態——**換月不會重播**。放進頁面裡就會每滑一次重播一次，
  水平的換頁動畫和垂直的落下在同一段時間裡打架。實測確認過：換月時新的那張
  是滿不透明、沒有垂直位移地水平滑進來的。
