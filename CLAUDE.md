...
**「記一筆」那片由上往下的是遮罩，不是那五列。** 五列自己仍然由右進（同設定
選單那套位移＋淡入＋錯開）—— 方向這件事一個畫面只講一次，遮罩已經講完了。

**那片遮罩要自己畫，不要用 `layout` 去縮節點的高度。** 縮節點那個寫法實測是
**從下緣往上長**的：節點縮短了，但裡面的內容仍然是滿版、也沒有被縮短後的邊界
切掉，畫出來是一片從底部長上來的灰，正好和要的方向相反。用 `drawBehind` 畫一塊
`size.height * cover` 高、貼著上緣的矩形就沒有這層轉換，畫多高就是多高。
**0.32 的壓暗同時是 blur 的退路**：`Modifier.blur()` 只在 API 31+ 有效
...
只有淡入**，方向完全交給背後那片由上往下蓋的遮罩講。面板原本也往上拉，
和遮罩剛好一個往下一個往上，兩個方向互相抵消，讀起來是「有東西在抖」
