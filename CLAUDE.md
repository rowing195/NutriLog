...
**曾經有兩條退路，都拿掉了**（`chooseActivity` 的註解裡有完整理由）：

- **總消耗扣掉基礎代謝**：兩個一千五百多的大數字相減去換一個一百多的小數字，
  兩邊的基礎代謝差幾個百分點，誤差就和答案同一個量級 —— 實測手錶記 153、這條路
  算出 **39**。而且它得引進「今天過了幾成」才扣得對，同一天在不同時刻讀會得到
  不同的歷史值。要重開這個題目之前先想清楚這兩件事。
- **步數換算**：固定係數乘出來的猜測值，和手錶實測混在同一個數字裡，使用者分不出
  哪天是量的、哪天是猜的。步數照樣讀、照樣存進 `daily_health_metrics` 給報表用，
  只是不再換算成大卡。
...
順手把兩條被拿掉的退路再釘死一次（理由見 `chooseActivity` 的註解）：

- **步數換算**：係數本身是準的（實測 1,998 步 × 64 kg × 0.0005 = 63，和三星自己算的
  63 一模一樣），壞的是**輸入** —— 拿健康連線那個 3,757 去乘會得到 120，目標無聲灌水
  57 大卡。乾淨的公式配髒的資料還是髒的。
- **總消耗扣基礎代謝**：那個總消耗不只含基礎代謝，還混了多個來源，減出來的東西沒有意義。
