...
- `StampButton` / `PillButton` / `TextAction` 的 `onClick` **不是最後一個參數**
  ...
  編譯錯誤訊息看起來跟型別不合無關。
- **`StampButton` 內部是 `fillMaxWidth()`，放進 Row 一定要給它 `weight(1f)`。**
  不給的話它吃掉整行，旁邊那個元件被壓成 0 寬、一個字一行地往下長，
  而 Row 的 `CenterVertically` 會把章擺到那一長條的正中間 —— 症狀是
  「兩個元件距離超遠」。而且**旁邊那段字越長才越明顯**，短的時候看起來像好了
  （健康連線的「上次同步」就是這樣：剛同步完是「剛剛」兩個字沒事，隔一陣子
  變成完整日期就又跑掉）。**更好的作法是照 `DriveSection` 那樣讓章獨佔一列、
  說明文字放它上面一行** —— 沒有 Row 就沒有這個問題。
...
- **點了會換到 app 內另一頁的選項（常吃／文字輸入、輸入營養素、條碼），選單不要收。**
  讓換頁的紙直接蓋住還開著、還模糊的選單。先收的話，遮罩（170ms）與模糊（220ms）
  都比紙（320ms）先退光，而那幾頁的紙由上往下蓋、最下面最後才蓋到 —— 面板與那顆章
  正好在最下面，會露出一條清楚的主頁，章還在裡面從 ✕ 轉回 +。換頁結束後今日頁整個被
  丟掉，回來時 `showAddSheet` 本來就是 false。**拍照／相簿例外**（`staysOnToday`）：
  它們先開外部 App、畫面還停在今日頁，要照舊收。新增選項時要決定它屬於哪一種。

  **但換頁到一半就按返回時，今日頁不會被丟掉** —— 轉場倒轉回同一個實例，選單還開著；
  慢一點返回則是重建、選單收好。同一個動作依快慢兩種結果，所以 `TodayScreen` 收一個
  `isCurrent`（`App.kt` 傳 `viewModel.screen == Screen.Today`），**重新變回目前畫面時
  一律收選單**。驗這條要「點選項後立刻返回」，用 `adb shell "input tap X Y; input keyevent
  KEYCODE_BACK"` 放在同一個 shell 裡才夠快，分兩次呼叫 adb 永遠趕不上轉場。
...
- **`WeekPageContent` 的借位 overlay 只在「日分頁器真的在跨週界」那段時間成立**
  ...
  **兩個分頁器交接的那幾幀本來就不同步，而且兩個方向要的行為剛好相反、
  overflow 卻都是 1，靠數值分不出來**：

  | 交接 | 誰在前面 | 借位該不該開 | 做錯的症狀 |
  |---|---|---|---|
  | 滑日子跨週界 | 日分頁器 | **要開**，直到 `scrollToPage` 接上 | 閃回舊的一週 |
  | 拖週長條換週 | 週分頁器 | **要關**，新那頁畫自己就對 | 閃上一週／滑進來是同一週 |

  所以分辨的不是數值，是**誰在動**。試過的兩種寫法都只修好一半：
  只看「這一頁是不是選到的那一週」會把滑日子那個交接關掉；
  只看週分頁器的 `settledPage` 則是拖週那個交接關不掉。

  **這是競速，不是每次都閃**（實測兩三次才一次），所以一定要反覆做十幾次才算驗過。
  四條路都要測：往後拖週、往前拖週、滑日子跨週界（借位本身還在不在）、
  以及箭頭換週。驗法是趡拖曳與剛放手時連拍 `adb exec-out screencap -p`，
  比對每一幀的 md5 —— settle 之後幾幀必須完全相同，還在變就是還有東西在動。

- **外部改日期時，距離超過一天就直接到位、不要 `animateScrollToPage`**
  （`LaunchedEffect(date)`）。換週時 `date` 一次跳七天，把那七頁演一遍會同時造出
  三個看起來不相干的症狀：選取框沿路掃過去（「週六跳週六」看起來先閃到週四週五）、
  週分頁器早就 settle 了這段還在跑（「換完之後還在抖」）、以及拉長了上面那個交接的
  空檔。相鄰的一天才值得演（點週長條旁邊那一格）；跨過一天以上的移動，
  中間那幾天不是使用者要看的東西。這一改連帶修好箭頭換週、月曆跳頁與
  「回到今天」—— 它們是同一個症狀的不同入口。
...
- **「回到本月」不在報頭裡，在畫面最底下，而且是空心章。** 它是一個動作而不是這個月
  ...
  **今日頁報頭的「回到今天」用同一套**（`HeaderRow` 的 `awayFromToday`），只是換成
  水平的：它和「2026 / 09」疊在同一個 Box 裡互換，一個滑出去另一個滑進來，方向跟著
  手指（往過去拖就從左邊進來）。疊在一起是為了讓那一格的寬度永遠是兩者的最大值 ——
  舊版是 `if/else` 直接換掉，旁邊那排圖示會因為換字而左右跳，而且離開今天就看不到
  年月了。**一樣要在 `graphicsLayer` 的 lambda 裡讀那個即時值**，寫在組合階段等於
  整個報頭每幀重組一次。
...
| 開出來的畫面 | 蓋的方向 |
|---|---|
| 報頭那排圖示：搜尋、月曆、設定（含設定的子頁） | **由下往上** |
| 「記一筆」那五個入口：常吃／文字輸入、輸入營養素、條碼、AI 確認 | **由上往下** |

兩邊各自朝著自己入口的反方向走：報頭那三顆站在頂端，開出來的東西從底下升上來；
「記一筆」那顆章站在右下角，開出來的東西從頂端蓋下來。返回就原路退回去
（由上往下來的往上收）。內容的方向同理：**設定由右進、搜尋由左進**，一左一右
分得開「剛剛按的是哪一顆」。**設定不管幾層都從右邊來**，往裡面走一層就再來一次。

實作是 `App.kt` 的 `Screen.coversDownward`。三件要記的：

- **方向綁在畫面身上，不是綁在「這次是誰開的」。** 編輯表單也能從紀錄列點進去、
  AI 確認與條碼會互相接力，那些路徑沒有「入口在哪個角落」可講。同一個畫面永遠
  從同一邊來，返回才保證是原路。新增畫面時要決定它屬於哪一邊。
- **方向由「動的是哪一張」決定，不是由前進／後退決定。** 前進時動的是新來的
  那張（`targetState`），後退時動的是正在走的那張（`initialState`）。拿錯一邊的話，
  從編輯表單返回會變成往下掉出去，而不是原路往上收。
- **這個「由上往下」講的是換畫面的紙，不是「記一筆」展開時那層遮罩。** 那層遮罩
  曾經被做成由上往下掃，結果是一片灰幕壓過整個今日頁，這是誤會 —— 它只是就地
  淡入配 blur。**0.32 的壓暗仍然不能拿掉**：`Modifier.blur()` 只在 API 31+ 有效
  （minSdk 26），Android 8～11 上就只剩它在講「後面暫時停用了」。

### 紙的濃度跟著它走了多遠，不是跟著它的邊緣

**整張紙的不透明度是一條隨行程走的曲線**（`sheetAlphaAt`）：剛起步時
只有 `SHEET_ALPHA_FROM`（50%），底下那頁還看得到；一路走一路變實，走到
...
（五列照舊由右滑進、面板本身只淡入），背後的遮罩也不掃任何方向。面板原本往上拉，
和當時往下掃的遮罩剛好相反、互相抵消，讀起來是「有東西在抖」。要加位移之前先問
一句：這個方向已經有人在講了嗎？
...
換算回正常速度省掉約 70ms。剩下那 3% 是紙最後才走到的那一條邊（由下往上的是
最上面、由上往下的是最下面），那時候紙早就實心了（八成行程就到頂），實測那一刻
舊畫面已經看不見。
...
- **疊放順序不能寫死「往裡走 1、往回走 0」。** `AnimatedContent` 的 zIndex 是每張在
  **進場那一刻**決定、退場時沿用的。寫死的話，設定 → API 管理 → 金鑰 → 返回
  （API 管理這次以 0 進場）→ 再返回（設定也是 0），兩張同分時後加入的畫在上面，
  API 管理往下滑走的整段動畫被實心的設定頁蓋住，看起來就是「沒有動畫」。
  兩層（設定 → 外觀 → 返回）驗不出來，要進過第三層再退兩次才會現形。
  現在是跟著導航累加（`topZ`）：往裡走 +1 疊在最上面，往回走 −1 壓在正在走的
  那張底下。**只要是「退回中間層之後再退一次」都會中**，同深度接力
  （常吃 → 表單 → 取消）也是同一件事。
...
**而目標本身也要挑對是「哪一天的」**：一律走 `Common.kt` 的 `Map<LocalDate, DailyTarget>.targetsOn(date, settings)`，
見〈過去的日子用過去的標準〉那節。
...
每天的值快取在 Room 的 `daily_health_metrics`（migration 2→3），週長條與月曆一打開
就要用，不能等健康連線慢慢回。

### 過去的日子用過去的標準

**改一次熱量目標不能把以前每一天都重新判一次。** 沒有這件事的話，上個月明明都在
目標內的日子會集體變紅（或反過來集體變綠），而使用者那幾天實際吃了多少一次都沒變 ——
那是我們的尺變了，不是他的行為變了。

所以每天的四格目標會存一份快照（`daily_targets`，migration 3→4），寫入規則只有兩條
（`NutriViewModel.rememberTodayTarget`）：

- **今天**：每次設定變動與每次開 app 都覆寫。今天是活的 —— 剛改完目標就要馬上看得到效果。
- **過去**：碰都不碰。

讀的時候一律走 `targetsOn(date, settings)`：**查不到就退回目前的設定**。這一條讓這個功能
之前的舊紀錄、以及 app 根本沒開過的那幾天，行為和以前一模一樣，不需要為歷史資料補寫任何東西。

一次訂全部（`observeDailyTargets()`）而不是跟著可見範圍跑：一天一列、五個欄位，
十年也才三千多列，為了它再養一條範圍查詢不划算。

**驗它不要等一天**。往 `daily_targets` 塞一列過去的日期、把門檻設得比那天的合計高，
月曆那格就該從紅色變回灰色、月摘要的「超標天數」跟著少一天；沒有那一列的日子則照現在的
設定判斷。實測過：7,647 大卡那天凍在 9,000 之後不再是紅的，而隔壁沒凍結的 2,005 大卡
那天照舊是橘的。

### 活動量只能算一次：係數或手錶，二選一

**活動係數的定義本身就含運動** —— UI 上「輕度」寫的是「每週運動 1–3 天」。所以
「係數目標 ＋ 當天的運動消耗」是把同一批熱量算兩次。實測：64 kg／159 cm 的人輕度
係數算出 2087，再加一趟 45 分鐘的跑步（約 408）變成 **2495**，而合理估算是久坐
1821 ＋ 運動回補一半 ≈ 2025。

所以熱量的底跟著**「讀取運動消耗」這個開關**走（`BmrCalculator` 的
`watchSuppliesActivity`）：

| 讀取運動消耗 | 熱量目標的底 | 每天再加 |
|---|---|---|
| 開 | 基礎代謝 × **久坐 1.2** | 量到的運動消耗 × 回補比例 |
| 關 | 基礎代謝 × 使用者填的活動係數 | 不加 |

**切換那個開關會重算四格目標**（`NutriViewModel.withRecalculatedTargets`）。不重算
就會留下一個「底是久坐、卻沒有東西補」或「底含運動、又再加一次運動」的組合。
沒填過身型的人不動他手填的數字。**蛋白質永遠照使用者填的活動量算** —— 它問的是
這個人活動量多大，不是熱量從哪裡來。

**沒動的日子就是那個底，不補任何東西**（`effectiveCalorieTarget`）。手錶壞掉或沒同步
的日子會因此偏低，這是刻意的取捨：使用者的前提是「讀得到才開這個功能，讀不到就關掉」，
而關掉會把目標算回含活動係數的那個數字。

### 運動熱量只回補一部分

`NutriSettings.exerciseEatBackPercent`（設定 → 每日目標的「運動熱量回補」，預設 **50**）。
穿戴裝置估熱量是它最不準的一項：系統性回顧給的誤差從 9% 到 40% 以上都有，某些裝置的
MAPE 破百；營養師的普遍建議是活動量設低一點、運動熱量只回補 25–50%。全額回補等於把
高估的那一段一起吃回去。

**今日頁那一行「運動 +N」顯示的是實際加進目標的量，不是手錶量到的量** ——
照量到的畫會變成「目標 ＋ 運動 ≠ 還有」。動掉多少、回補幾成，點進明細才看得到。

### 步數只算「超出久坐基準」的那一段

三星不寫活動大卡、總消耗又是一條基礎代謝的直線，所以**沒開運動紀錄的日子裡，
步數是唯一有真實資料的訊號**。但整筆加進目標是錯的：

```
目標的底 = 基礎代謝 × 久坐 1.2
                        └─ 多給的那 0.2 本來就是「日常走動 + 食物熱效應」
```

「久坐 1.2」的定義不是躺著不動，是「沒有規律運動的正常人」—— 走去廁所、走去買午餐
都在裡面。不扣就是把同一批熱量算兩次，和「活動係數目標＋手錶運動」那個 2,495 的錯誤同一類。

所以（`dailyMovementAllowance` 與 `chooseActivity`，都是純函式、有測試）：

```
內含額度 = 基礎代謝 × 0.2 − 目標 × 0.1     ← 扣掉跟走路無關的食物熱效應
日常步數 = 總步數 − 運動場次裡的步數      ← 跑步那段已經算在場次熱量裡
活動消耗 = max(0, 日常步數換算 − 內含額度) + 場次熱量
```

實際的量級（基礎代謝 1,620、目標 1,944、64 kg，內含額度約 130 大卡 ≈ 4,000 步）：
...
- **有活動大卡時步數完全不參與。** 兩者估的是同一件事（一整天的走動），永遠是二選一。
  三星不寫所以這道門在這支手機上一次都不會進去 —— 它擋的是別的手機。
- **運動場次裡的步數要扣掉**（`WorkoutSessionItem.steps`），不然跑步那三十分鐘會被算兩次。
- **`ActivitySource.STEPS` 和 coworker 那版同名但不同意思**：那邊是整天步數直接換算，
  這裡是超出量。不要因為「以前拿掉過」就把它再拿掉一次。
- **「讀不到」與「讀到了但沒超出」要分開講**（`NO_SURPLUS_REASON`）。兩件事往完全不同的
  方向修：一個是去調權限，一個是多走兩步。運動明細那張面板的「走路」那一列
  **就算加進目標的是 0 也要畫**，那正好回答「我明明有走，為什麼是 +0」。

**內含額度那個值有誤差，不要當成精確值。** 食物熱效應抽 10% 是標準值，但活動係數到底
怎麼拆成「熱效應 vs 走動」文獻沒有統一講法，換算成步數是四千步、正負一千。
落到目標上是 ±16 大卡再乘回補比例，可以接受。

### 手錶怎麼戴決定哪一種資料算數

`WatchWearMode`（同一頁的「手錶配戴方式」）**不影響目標怎麼算**，只決定採用哪一種資料：

| | 採用 |
|---|---|
| 整天配戴 | 全日活動消耗（運動場次比它大時改用場次） |
| 只有運動時戴 | 只有運動場次那幾筆 |

**只有運動時戴的人不能用全日活動消耗。** 那個數字只涵蓋戴著的那幾小時，拿來當一整天
會低估；實測只戴兩小時、手錶記 153。

**三星只把「總消耗」寫進健康連線，日常的活動消耗沒有完整寫進去**（活動消耗只在運動
場次裡才有）。所以在三星手機上「整天配戴」多半拿不到全日活動消耗，真正讀得到的是
使用者按下「開始運動」記下來的場次。

**總消耗扣掉基礎代謝那條退路拿掉了，不要加回來**（`chooseActivity` 的註解裡有完整理由）：
兩個一千五百多的大數字相減去換一個一百多的小數字，兩邊的基礎代謝差幾個百分點，
誤差就和答案同一個量級 —— 實測手錶記 153、這條路算出 **39**。而且它得引進
「今天過了幾成」才扣得對，同一天在不同時刻讀會得到不同的歷史值。
**在這支手機上它連第一步都站不住**：診斷讀到的總消耗是 `1,426（0 筆）`，
沒有任何 app 寫過它，而且實測「到現在」= 892 = 1,426 × 62.57%（15:01 已過的時間比例）
—— **它累加得完美地線性，裡面一點活動成分都沒有**，減完什麼都不剩。

讀不到就回 `ActivitySource.NONE` 並講原因，不要給一個猜的數字 —— 運動明細那張面板
一定要講「來源」。**運動場次內部那段「總消耗扣基礎代謝」留著**：它只算那一場的
時間窗（幾十分鐘），誤差和全日那條差一個量級。

### 對不上的時候看「診斷」，不要用猜的

設定 → 健康連線最底下有一顆「讀取診斷資訊」（`HealthConnectSync.diagnose`），列出今天
從健康連線讀到的**原始值**：活動大卡、總消耗、步數、運動場次、四個權限各有沒有，
以及正式路徑挑了哪一個。同一份也寫進 logcat：

```bash
adb logcat -s HealthDiagnostics
```

**「無資料」和「0」在型別上就分開**（`Double?` / `Long?`）：前者是三星根本沒寫進來
或沒授權，後者是有資料但那天沒動 —— 兩件事往完全不同的方向修，混在一起就只能猜。

**採用的那個結果直接呼叫 `readDailyActivity`**，不另外算一份。診斷和實際行為各寫一套
的話，畫面上說的和目標用的遲早會漂掉，而那是最難查的一種 bug。

實際救過的例子：三星自己的 app 顯示活動消耗 155、總消耗 1912，健康連線裡卻一筆活動
大卡都沒有，而四個讀取權限全開 —— 沒有這張表只會一直懷疑是不是權限沒給。

**模擬器上不要真的授權。** 驗到「權限畫面跳得出來、按返回之後 app 講得出沒拿到權限」
就好；運動消耗的畫面用 `run-as` 往 `daily_health_metrics` 塞一列假資料來驗，驗完刪掉。
診斷畫面在沒授權的模擬器上也驗得到：每一列都會是「無資料」，權限那列四個都是「無」。

### 健康連線是共用池，不是 Samsung Health 的鏡子

數字和手錶的 app 對不上時，第一個要懷疑的**不是我們讀錯**，而是**那個池子裡不只一家
在寫**。實測同一天同一刻：

| | Samsung Health | 健康連線讀到 |
|---|---|---|
| 步數 | 1,998 | **3,757** |
| 熱量 | 994（總計燃燒）| **1,426**（總消耗）|

步數幾乎是兩倍，因為**手機自己也在寫一份**（健康連線 → 資料與存取權 → 步數 → 存取權，
會看到「Samsung Health」和一個以裝置名稱命名的來源）。健康連線讀合計時本來會照
「應用程式優先順序」去重，但那份清單是使用者自己排的，沒排到的來源就會疊加。

**Samsung Health 那一份已經是手機＋手錶去重後的合併值。** 三星的 app 自己就在讀手機的
計步器，沒戴錶只帶手機走的步數一樣算得到（官方說明：手錶上顯示的是手錶自己記的，
手機上顯示的才是合併後的總數，而合併過程保證不重複）。所以優先順序**選 Samsung Health
不會漏掉任何步數** —— 手機那個來源是同一批步伐的另一份副本，不是額外的涵蓋範圍。

拆開來看就很清楚，兩份是**各自獨立數的**，不是互補的兩半：

```
Samsung Health  1,998   手錶＋手機合併去重後的完整值
裝置本身         1,790   健康連線自己另外數的一份
                ─────
合計讀到         3,788   ≈ 實際讀到的 3,757
```

手機那份比較少，是因為它只數得到「帶著手機走」的那些；Samsung 那份還多了手錶記到、
手機沒記到的（手機放桌上時的走動）。兩份**大量重疊**。

**而這個來源是我們自己要權限時才冒出來的** —— Google 的說明白紙黑字：「只要有 app
取得步數讀取權限，手機就開始收集步數」。也就是說這個坑會跟著每一個授權過的使用者，
不是這台裝置的個案。要清掉不必動任何權限，有兩條路：

- **排除出合計**（保守，照樣記錄）：健康連線 → 管理資料 → 資料來源與優先順序 →
  活動 → 找到裝置 → 更多 → 移除
- **直接關掉**（乾淨）：健康連線 → 裝置 → 選裝置 → 關掉「步數」

三星到底寫了什麼進健康連線（官方文件，和實測一致）：

| Samsung Health 主頁 | 有沒有進健康連線 |
|---|---|
| 活動消耗熱量 | ❌ 官方明講 activity tracker 的資料不同步 |
| 總計燃燒熱量 | ⚠️ **沒有任何 app 寫過，是健康連線自己算的**（見下）|
| 步數 | ✅ 但會和手機自己那份疊加 |
| 運動場次 | ✅ **唯一乾淨的一項** |

**所以診斷畫面上的「活動大卡 無資料」不是 bug，是三星根本不寫這個型別。** 這也正是
正式路徑只認活動大卡與運動場次的原因：真正穩定的只有運動場次。

**「總消耗」更糟：它連寫入者都沒有。** 診斷讀到 `1,426（0 筆）` —— 合計有數字，
原始紀錄卻是零筆，健康連線的資料清單裡也根本沒有「卡路里」這個類別。也就是說那個值
是系統自己從身高體重之類的東西推出來的，官方文件沒有任何一處說明它怎麼算。
**一個講不出來源的數字，不能拿去減基礎代謝再當成活動量** —— coworker 那條退路
（見 `chooseActivity` 的註解）在這支手機上連第一步都站不住。

步數則是另一種：`3,647（57 筆）`，兩個寫入者 `com.sec.android.app.shealth` 與
`com.android.healthconnect.phone.<一長串 hash>`，後者就是系統自己那份。

這件事直接影響步數能不能拿來算熱量（見下一節）：**係數本身是準的**（實測 1,998 步 ×
64 kg × 0.0005 = 63，和三星自己算的 63 一模一樣 —— 三星那個「活動消耗熱量」本身
就是步數換算出來的），**壞的是輸入** —— 沒排好優先順序的話拿到的是兩個來源疊加的
3,757，乘出來會把目標無聲灌水。乾淨的公式配髒的資料還是髒的，
**所以優先順序排好是步數能用的前提**。
...
熱量的活動係數看「讀取運動消耗」開著沒有（開著就退到久坐 1.2，見上一節），
**蛋白質永遠照使用者填的活動量**。
身型本身跟著一起存（`profile*` 欄位），週報要用體重判斷蛋白質、基礎代謝要算每日消耗。

**蛋白質跟著活動量走，不是只看目標**（`ActivityLevel.proteinPerKg`：久坐 1.0 →
非常高 1.8 g/kg，減脂與增肌各再 +0.2，封頂 2.0）。對方原本是只看目標的固定倍率、
維持一律 **1.7 g/kg** —— 那是 ISSN 給「有在訓練的人」1.4–2.0 那一區的上緣，套在
久坐、只想維持體重的人身上等於逼他每天喝高蛋白（64 kg 會算出 108 g）；而且久坐與
非常高活動量會得到一模一樣的數字，活動量那一欄形同白填。`BmrCalculatorTest` 有一條
專門釘這件事。

**碳水固定佔 55%，脂肪吃差額**（原本相反：脂肪固定 25%、碳水吃差額）。相反的話，
蛋白質一降，省下來的熱量一克不剩全部跑到碳水，比例會被推到建議範圍 50–65% 的上緣
（實測 58.7%）。碳水是三者裡份量最大的一項，讓它當定值比較穩。**但脂肪有下限
（20%）**，蛋白質高到塞不下時讓位的是碳水 —— 脂肪太低會影響荷爾蒙與脂溶性維生素吸收。
...
- **拿掉一款比新增一款危險**（實際拿掉過「碗」）。兩件事要先做到：
  一、`SettingsStore` 的 Json 要開 `coerceInputValues` —— `ignoreUnknownKeys` 只管認不得的
  **欄位**，認不得的 **enum 值**會直接拋例外，而整包設定是一段 JSON，一拋就被
  `getOrDefault(NutriSettings())` 接走，**連 API 金鑰與每日目標一起變回預設**。
  二、**使用者當下正選著那一款的話，更新完會完全沒有桌面入口** —— 舊的 alias 不在了，
  其餘 alias 的「停用」狀態卻留在系統那一側，等於沒有任何 MAIN/LAUNCHER 可以啟動。
  要 app 自己跑過一次才會把預設那顆重新開起來，而打不開就跑不了。
  只有在確定沒人選著它的時候才適合直接拿掉。
