...
- **兩個分頁器都預設 `beyondViewportPageCount = 0`**，滑到還沒保留住的頁面
  ...
  不能只測「換頁不閃了」就算過。
- **`WeekPageContent` 的借位 overlay 只能給「目前選到的那一週」那一頁算**
  （`isSelectedWeek`）。它的用途是日分頁器拖過週界時先讓鄰週滑進來，而那件事
  只發生在選到的日子所在的那一週；其他頁的 `pillPosition` 本來就差了整整一週
  以上，`coerceIn(0f, 1f)` 會直接飽和成 1 —— 那一頁於是把自己整個推出畫面、
  改畫鄰週。**症狀是拖動週長條換週時，滑進來的是同一週，放開手指才補跳一次**
  （往後拖時下一週那一頁的 `pillPosition` 正好是 −1，畫出來的是「自己的前一週」，
  也就是使用者眼前這一週；等 settle 後 `weekStart` 才變、日分頁器才跟上，
  借位歸零，正確的那一週才出現）。這條和上面那幾個是同一個家族的錯：
  **把一個分頁器的即時位移拿去餵另一個東西時，永遠要先問「這一頁真的和那個
  位移有關嗎」**。改這段之後三條路都要測：往後拖、往前拖、以及日分頁器跨週界
  （借位本身還在不在）。驗法是趁拖曳中連拍 `adb exec-out screencap -p`，
  看滑進來的那幾格日期對不對 —— 放開手指之後畫面是對的，看不出這個 bug。
