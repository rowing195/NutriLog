...
**升上來那張紙的前緣不是一條硬邊，是一段 120dp 的漸層**（`EDGE_FADE_DP`）：
最前面 50%（`EDGE_ALPHA`）不透明度，往回收到完全不透明。硬邊在快速滑過時
會看成一條掃過畫面的線，那條線本身變成主角；羽化之後讀到的是「紙壓上來」
而不是「有東西掃過去」。

作法是 `drawWithContent` 疊一層 `Brush.verticalGradient` 配 `BlendMode.DstIn`，
外面那層要 `CompositingStrategy.Offscreen` —— 沒有離屏圖層的話 `DstIn` 會拿
整個視窗當目的地，結果是把底下的畫面一起挖掉。

**羽化要綁在「還沒蓋滿」上**（和 `LocalScreenEntered` 同一個 `covered`），
不是綁在轉場上：蓋滿之後那張紙就是這個畫面本身，還留著漸層等於畫面頂端
永遠有一段淡掉的區域。返回時新出現的畫面 `covered` 一開始就是 true，
所以它不會有羽化 —— 那是對的，返回時動的是上面那張紙，不是它。
...
- **返回那個方向要另外放行。** 返回時新畫面是 `EnterTransition.None`（動的是上面
  那張紙），它的 `currentState` 一開始就不是 `PreEnter`，不能讓它跟著等 0.97 ——
  所以條件是「`currentState` 已經翻面 **或** 進度過門檻」，兩者取聯集。
- **進度值要包 `derivedStateOf` 再拿去比大小。** 它每一幀都在變，直接讀等於整個
  畫面每幀重組一次；包成布林值之後只有真的翻面才通知讀它的人。和常吃頁讀
  `WindowInsets.ime` 那條是同一個陷阱。
...
- **底色那一層不能吃內容的 alpha。** 升上來的必須是一張不透明的紙，
  否則「蓋住」會變成「洗淡」—— 兩層一起看得到。
