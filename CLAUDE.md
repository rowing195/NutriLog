...
| 右下角那顆「記一筆」的五個選項 | **由上往下** |

兩邊各自朝著自己入口的反方向走，對應得起來。內容的方向同理：
**設定由右進、搜尋由左進**，兩個都是報頭圖示開出來的畫面，一左一右分得開
「剛剛按的是哪一顆」。**設定不管幾層都從右邊來**，往裡面走一層就再來一次。
...
**方向感一個畫面只講一次。** 右下角那顆「記一筆」展開時，**面板自己不做位移，
只有淡入**，方向完全交給背後那片由上往下蓋的模糊講。面板原本也往上拉，
和背後那片剛好一個往下一個往上，兩個方向互相抵消，讀起來是「有東西在抖」
而不是「蓋下來」。要加位移之前先問一句：這個方向已經有人在講了嗎？
