...
| 食物庫聚合 | 大杯／中杯珍奶分成兩列且熱量不同（480 / 320）；120 天前那筆被 90 天窗口排除 |
| 最近一次的數值 | 同名同份量三筆（111/555/999），聚合取到 999；拿掉 MAX() 的對照組取到 111 |
| 多關鍵字 AND | `bubble large` 精準命中大杯那筆（跨欄位、大小寫不敏感）；`bagel large` 得 0 筆 |
| 打字切換 | 一輸入分頁標籤收起換成結果；清空回到兩頁 |
| 食物庫 → 加入 | 點一項跳面板 → 加入 → 預填表單（標題「新增食物」）→ 存 → 入帳 |
| 搜尋結果點列 | 跳到 8/18 那天，總和 835 |
| 搜尋結果「＋」 | 帶出 Medium 那筆的精確數值（320/5/8/60），不是改到原紀錄 |
| 加入的目標日期 | 從 8/18 進搜尋加入 → 落在 8/18（835→1155），畫面有明講 |
...
### 食物庫的「最近一次數值」靠的是 SQLite 的一個明文保證

`observeFrequentFoods` / `observeRecentFoods` 用 `GROUP BY name, servingText`
聚合，但營養素那幾個**裸欄位**要取自最後一次吃的那一筆。這靠的是 SQLite
的規定：聚合查詢裡剛好有一個 `max()` 或 `min()` 時，同一列的裸欄位
會取自那個極值所在的資料列。

**少了 `MAX(loggedAt)` 就會靜默取到隨便一筆。** 已實測：同名同份量三筆
熱量 111/555/999，有 MAX() 取到 999，拿掉就取到 111。改這兩個查詢時
不要順手把 MAX() 拿掉。
