...
<img src="art/cover.png" width="100%" alt="NutriLog Cover Banner"/>
<br><br>

<img src="app/src/main/res/mipmap-xxxhdpi/ic_launcher_foreground.png" width="120" alt="NutriLog Project Logo"/>
...
- [功能](#功能)
    ...
    - [份數縮放](#份數縮放)
    - [四種輸入方式](#四種輸入方式)
    - [點餐收據與全場景影像辨識](#點餐收據與全場景影像辨識)
    - [AI 健檢報告中心（OpenRouter / Nemotron）](#ai-健檢報告中心openrouter--nemotron)
    - [Samsung Health / 健康連線](#samsung-health--健康連線)
    - [歷史（月曆）](#歷史月曆)
    - [搜尋與個人食物庫](#搜尋與個人食物庫)
    - [匯出／匯入 CSV](#匯出匯入-csv)
...
Android 每日飲食營養素紀錄器（Kotlin + Compose）。app 顯示名稱與專案代號皆為 NutriLog。

**Why NutriLog?** 市面上的飲食紀錄 app 幾乎都要你先開帳號、再把三餐上傳到別人的伺服器。
這支不用：沒有後端、沒有帳號，紀錄全部躺在你自己的手機裡。

- 🔒 **完全離線** — 唯一的對外連線是影像辨識、條碼查詢與 AI 報告等公開 API，兩者都是你主動觸發或明確授權才會發生。
- 🧾 **全場景影像辨識（實體食物 ＋ 點餐收據／菜單／外送單）** — 拍一盤菜或直接拍麻辣燙、熱炒點餐單與手搖飲明細皆可，自動完成品項 OCR、數量乘積換算、0元滿額贈品納入與非食物雜訊過濾。
- 📊 **AI 健檢報告中心（週報 & 月報 & 卡路里自適應推薦）** — 串接 OpenRouter（預設 Nemotron 3 Ultra 550B，相容 NVIDIA NIM），結合手錶實測運動大數據，每週日自動背景結算，提供下週熱量與三大營養素個人化目標推薦，並支援跨週、跨月對比覆盤。
- ⌚ **Samsung Health 雙向同步** — 透過 Health Connect 自動將飲食與營養素寫入三星健康（支援早/午/晚點心精確時段分類），並自動讀取手錶或手機記錄之每日活動消耗卡路里，即時計算淨熱量收支。
- 🍱 **多通道輸入路徑** — 自己填數字、拍照或拍收據辨識、打一句話交給 Gemini 估、掃商品條碼查 Open Food Facts。
...
- 🔑 **極簡權限架構** — 本體無需宣告 `CAMERA` 或敏感權限，相機在系統相機與 Play 服務中執行。

---

## 最近更新亮點

### 1. 點餐收據、外送單與菜單多模態熱量拆解 🧾
針對台灣外食常見的麻辣燙、滷味、熱炒、鹹酥雞等混合食材料理，升級 `GeminiClient` 多模態視覺提示詞：
- **數量乘積自動計算**：品項自帶單位與「數量」欄位自動相乘（例如 `鳥蛋3個` 數量 `2` ➔ 自動計算為 6 顆，份量註記為 `6顆 (2份)`，熱量按 6 顆計算）。
- **滿額贈送品項識別**：收據上的 `滿200送豬肉 0元 數量1` 不會被當作折扣忽略，自動作為實體食物（豬肉片 1 份）納入營養估算。
- **料理情境吸油與鈉估算**：結合整單烹調情境（如麻辣燙、老油條吸湯吸油特性）進行符合台灣在地口味的熱量估算。
- **抗行程回收之相機金鑰三重防線**：建立「記憶體 State ➔ 同步 SharedPreferences 快取 ➔ DataStore 磁碟備援」三重防線，徹底解決 Samsung 等高記憶體佔用相機在返回時因行程回收重啟導致誤判「尚未設定 API Key」的競爭條件。

### 2. AI 健檢報告中心（週報 & 月報 & 卡路里自適應推薦）📊
- **OpenRouter / NVIDIA NIM 深度健康分析**：預設串接 OpenRouter 上高速免費旗艦推理模型 `nvidia/nemotron-3-ultra-550b-a55b:free`（亦相容 NVIDIA NIM 官方端點），結合一週/全月飲食真實紀錄與手錶實測運動大數據，產出專業教練級深度體態覆盤。
- **週日自動結算排程**：透過 WorkManager 在每週日自動為使用者結算當週健檢報告。
- **自適應熱量與三大營養素目標推薦**：根據真實代謝與熱量收支，生成下週每日建議攝取目標，並支援一鍵套用。
- **跨期宏觀對比**：支援 `vs 上週` 與 `vs 上月` 之熱量、運動消耗、TDEE 與步數變化對比。
- **離屏後台非同步運算**：啟動分析後可自由返回記錄三餐或查看歷史，頂部懸浮進度條即時提示分析狀態。

### 3. Samsung Health / Health Connect 雙向健康連線 ⌚
- **飲食雙向同步**：每日飲食熱量與微量營養素自動寫入 Samsung Health，並依用餐時段自動精確映射至三星健康「早餐、午餐、晚餐、早點心、午點心、晚點心」。
- **手錶運動消耗熱量自動讀取**：自 Samsung Health 即時讀取手錶記錄之 Active Calories，在首頁一併展示「已攝取熱量、運動消耗熱量與當日淨攝取」。
		...
		<blockquote>
			...
			<thead>
				...
				<tr style='border-bottom: 1px solid #eee;'>
					...
					<td style='padding: 8px;'>BMR 與 TDEE 估算公式（Mifflin-St Jeor 經驗公式、性別/身高/體重/年齡係數與活動量換算）。</td>
				</tr>
				<tr style='border-bottom: 1px solid #eee;'>
				...
				<tr style='border-bottom: 1px solid #eee;'>
					...
					<td style='padding: 8px;'>與 Android Health Connect（Samsung Health）雙向同步，支援飲食與微量營養素寫入、早/午/晚點心時段對齊與手錶運動消耗熱量即時讀取。</td>
				</tr>
				<tr style='border-bottom: 1px solid #eee;'>
					...
					<td style='padding: 8px;'>月度宏觀數據彙總聚合器，計算全月總攝取、日均熱量、總運動消耗與實測 TDEE 趨勢。</td>
				</tr>
				<tr style='border-bottom: 1px solid #eee;'>
					...
					<td style='padding: 8px;'>本機儲存每月 AI 健康月報快照與全月宏觀體態數據。</td>
				</tr>
				<tr style='border-bottom: 1px solid #eee;'>
				...
				<tr style='border-bottom: 1px solid #eee;'>
					...
					<td style='padding: 8px;'>週度數據彙總聚合器，彙整 7 天實際熱量攝取、手錶運動消耗、淨熱量與體重變化趨勢。</td>
				</tr>
				<tr style='border-bottom: 1px solid #eee;'>
					...
					<td style='padding: 8px;'>本機儲存每週 AI 健康週報快照、7 天實測數據彙總與自適應推薦目標。</td>
				</tr>
			</table>
		</blockquote>
	</details>
		...
		<blockquote>
			...
			<thead>
				...
				<tr style='border-bottom: 1px solid #eee;'>
					...
					<td style='padding: 8px;'>每日生理健康指標實體（快取自 Health Connect 之體重、運動消耗、步數與靜止心率）。</td>
				</tr>
				<tr style='border-bottom: 1px solid #eee;'>
		...
		<blockquote>
			...
			<thead>
				...
				<tr style='border-bottom: 1px solid #eee;'>
					...
					<td style='padding: 8px;'>照片與文字描述的營養估算。支援實體餐點與點餐收據／菜單多模態 OCR。以 `responseSchema` 強制結構化 JSON 輸出，自動重試 5xx 與網路逾時。</td>
				</tr>
				<tr style='border-bottom: 1px solid #eee;'>
				...
				<tr style='border-bottom: 1px solid #eee;'>
					<td style='padding: 8px;'><b><a href='https://github.com/rowing195/NutriLog/blob/main/app/src/main/java/com/watson/nutrilog/data/net/NvidiaClient.kt'>NvidiaClient.kt</a></b></td>
					<td style='padding: 8px;'>AI 健檢報告客戶端，預設串接 OpenRouter 高速免費旗艦模型 Nemotron 3 Ultra 550B（相容 NVIDIA NIM 端點），進行週報、月報深度醫學評估與自適應熱量動態推薦。</td>
				</tr>
				<tr style='border-bottom: 1px solid #eee;'>
		...
		<blockquote>
			...
			<thead>
				...
				<tr style='border-bottom: 1px solid #eee;'>
					...
					<td style='padding: 8px;'>外觀模式（系統/淺色/深色）、雙 AI 金鑰（Gemini 與 OpenRouter / NVIDIA NIM）、Samsung Health 雙向同步開關、每日營養目標與 BMR 試算、進階營養素開關與 CSV / Drive 雲端備份。</td>
				</tr>
				<tr style='border-bottom: 1px solid #eee;'>
					...
					<td style='padding: 8px;'>BMR / TDEE 試算彈窗：輸入性別、年齡、身高、體重與活動量動態試算基礎代謝，支援一鍵將計算結果帶入每日目標。</td>
				</tr>
				<tr style='border-bottom: 1px solid #eee;'>
					<td style='padding: 8px;'><b><a href='https://github.com/rowing195/NutriLog/blob/main/app/src/main/java/com/watson/nutrilog/ui/WeeklyReportScreen.kt'>WeeklyReportScreen.kt</a></b></td>
					<td style='padding: 8px;'>AI 健檢報告中心畫面：支援週報／月報切換、實測 TDEE 與淨熱量數據卡片、教練級深思維報告展示、一鍵套用推薦目標與離屏後台運算通知。</td>
				</tr>
				<tr style='border-bottom: 1px solid #eee;'>
	...
	<details>
		...
		<blockquote>
			...
			<thead>
				...
				<tr style='border-bottom: 1px solid #eee;'>
					<td style='padding: 8px;'><b><a href='https://github.com/rowing195/NutriLog/blob/main/app/src/main/java/com/watson/nutrilog/work/SundayReportWorker.kt'>SundayReportWorker.kt</a></b></td>
					<td style='padding: 8px;'>每週日背景定時結算週報 WorkManager 排程。<br>- 每週日定時觸發，結合當週飲食 Room 紀錄與 Health Connect 運動數據。<br>- 調用 OpenRouter（Nemotron 3 Ultra 550B）/ NVIDIA NIM 非同步生成 AI 健檢週報並快取至本地。</td>
				</tr>
			</table>
		</blockquote>
	</details>
...
| **拍照辨識** | 拍照或自相簿選取（支援實體餐點或點餐收據／菜單／食品標籤） → 壓縮長邊至 1024 px → Gemini 結構化辨識 → **確認畫面**逐項勾選與微調後入庫。 |
...
針對外食族的真實生活情境，NutriLog 支援「全場景多模態辨識」：
- **實體餐點 vs. 點餐明細自動辨識**：無論拍整盤食物、便利商店微波食品背面的營養標示表，還是麻辣燙、熱炒、鹹酥雞的點餐收據、外送明細，AI 皆能精準判斷模式。
- **品項規格與數量倍率連動**：
  - 收據中若出現「鳥蛋3個」數量「2」，系統自動計算為 6 顆，標註份量為「6顆（2份）」，熱量與三大營養素依 6 顆計算。
  - 規格如「火鍋豆腐半盒」數量「1」，自動推估為半盒約 150g 之營養值。
- **滿額贈送與 0 元品項納入**：如「滿200送豬肉 0元 數量1」，系統不會因金額為 0 而略過，忠實還原為「豬肉片 1 份」供使用者記錄。
- **在地料理情境吸油與鈉估算**：識別出「麻辣燙、老油條、微辣」時，自動納入油條吸油特性與湯底鈉含量，使外食紀錄更貼近真實吸收。
- **非食物雜訊嚴格過濾**：自動剔除「內用/外帶」、「桌號」、「小計金額」、「總計: 240」、「OrderNumber」等與營養無關文字。
- **多品項勾選確認與份量微調**：辨識結果在 `ReviewScreen` 逐項展開，可自由取消未吃的項目或微調各單品份數倍率後一鍵儲存。

### AI 健檢報告中心（OpenRouter / Nemotron）

結合「手錶實測運動大數據」與「本機飲食紀錄」，提供教練級深思維覆盤：
- **雙報告體系**：
  - **週報 (Weekly)**：回顧本週總攝取大卡、手錶實測總消耗、實測每日 TDEE、淨熱量收支與步數運動次數。
  - **月報 (Monthly)**：宏觀回顧整月體態趨勢，並提供 `vs 上月` / `vs 上週` 之收支比較。
- **自適應熱量與三大營養素推薦**：由 OpenRouter 上的 NVIDIA Nemotron 3 Ultra 550B（或 NVIDIA NIM）分析實際代謝赤字與盈餘，推薦下週每日目標，並支援一鍵覆蓋為 App 每日目標。
- **週日自動結算**：背景排程在每週日自動為使用者產生當週結算報告。
- **離屏後台非同步執行**：啟動分析後可離開該畫面記錄飲食或查看歷史，頂部浮動膠囊即時反饋運算進度與完成提示。

### Samsung Health / 健康連線

透過 Android 官方 Health Connect 達成無感健康雙向連線：
- **飲食與營養素寫入**：將三餐與點心熱量、三大營養素寫入 Samsung Health。
- **精準點心時段對齊**：依紀錄時間自動對齊三星健康「早點心（10:30前）、午點心（15:00前）、晚點心（21:00前）」分類。
- **手錶運動消耗熱量即時讀取**：自動獲取 Galaxy Watch 或手機實測之活動消耗熱量（Active Calories），於首頁計算淨攝取。
...
NutriLog 支援雙 AI 引擎配置，依使用者需求分別填入：

### 1. Google Gemini API key（影像辨識與文字估算）
- **功能**：拍照辨識、點餐收據／菜單 OCR、常吃／自然語言文字估算。
- **取得方式**：前往 [Google AI Studio](https://aistudio.google.com) 免費申請。
- **設定位置**：開啟 App → 右上角 **設定** → 貼入 **Gemini API key**。
- **支援模型**：預設推薦 `gemini-3.7-flash`，亦可選用 `gemini-3.5-flash-lite`。

### 2. OpenRouter API key（AI 健檢報告中心）
- **功能**：AI 健檢週報、月報宏觀分析與個人化自適應熱量目標推薦。
- **取得方式**：前往 [OpenRouter](https://openrouter.ai) 申請 API Key。
- **設定位置**：開啟 App → 右上角 **設定** → 貼入 **OpenRouter API key**。
- **支援模型**：預設使用高速免費旗艦模型 `nvidia/nemotron-3-ultra-550b-a55b:free`（亦具備向下相容，支援填入 NVIDIA NIM `nvapi-…` 金鑰）。

> 🔒 **安全保證**：所有 API Key 僅安全儲存於本地 DataStore 與抗重啟快顯層，**絕不會打包進 APK 或上傳第三方伺服器**。
...
- 支援實體食物照片、點餐收據／菜單／外送單 OCR 與文字描述營養估算。
- 內建 5xx / 逾時自動指數退避重試 3 次。

### OpenRouter API（AI 健檢報告中心）

```
POST https://openrouter.ai/api/v1/chat/completions
```

- API Key 走 `Authorization: Bearer {apiKey}` HTTP Header。
- 專為大模型深思維健康報告設計（預設旗艦免費模型 `nvidia/nemotron-3-ultra-550b-a55b:free`）。
- 附帶 `HTTP-Referer` 與 `X-Title` Header，支援結構化自適應推薦標籤提取。
- 具備智慧端點識別，若填入 `nvapi-…` 自動相容切換回 NVIDIA NIM 端點。
